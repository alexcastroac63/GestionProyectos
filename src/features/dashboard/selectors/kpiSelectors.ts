...
export function getActiveSprintsItems(
  ...
  const safeItems = workItems || [];
  return safeItems.filter(w => w.project_id === projectId && w.sprint_id === activeSprintId);
}
...
export function getTotalPoints(workItems: WorkItem[] = []): number {
  const safeItems = workItems || [];
  return safeItems.reduce((acc, current) => acc + (current.story_points || 0), 0);
}
